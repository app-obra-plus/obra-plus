...
export class ChatService {
  ...
  async createChat(chat: CreateChatDto): Promise<ChatResponseDto> {
    const existingChat = await this.validateExistingChat(chat);

    if (existingChat) {
      return this.getChatById(existingChat.id);
    }
  ...
  ): Promise<PaginatedResponse<ChatSummaryDto>> {
  ...
  async getAllMessagesFromChatPage(
    chatId: string,
    userId: string,
    params: PaginationParamsBase
  ...
  private isValidISODateStrict(dateStr: string) {
    ...
    if (!(isoRegex.test(dateStr) && !Number.isNaN(Date.parse(dateStr)))) {
      throw new BadRequestError(
        "O parâmetro 'after' é obrigatório e deve estar no formato ISO 8601."
      );
    }
  }

  private async validateExistingChat(chat: CreateChatDto) {
    const possibleChats = await prisma.chat.findMany({
      where: {
        participants: {
          every: {
            userId: {in: chat.participantsId}
          }
        }
      },
      include: {
    ...
    const existingChat = possibleChats.find((c) => {
      const chatUserIds = c.participants.map((p) => p.userId).sort();
      const payloadUserIds = [...chat.participantsId].sort();
      return (
        chatUserIds.length === payloadUserIds.length &&
        chatUserIds.every((id, idx) => id === payloadUserIds[idx])
      );
    });
